...
describe("getVendingResult", () => {
  const validCases = [
    ...
    // Edge Cases
    [
      "Far too much money provided",
      "50 50 50 50 50 50 50 50 50 50 50 50 50 20 20 20 20 20 20 20 20 20 20 10 10 10 10 10 10 5 5 5 5 2 2 2 2 1 1 1 1 A",
      "A",
      "50 50 50 50 50 50 50 50 50 50 50 50 50 50 50 50 20 20 5 2",
    ],
    ["Just one cent over", "50 50 20 5 1 1 B", "B", "1"],
  ];

  it.each(validCases)(
    "should return the product bought and the correct change with the least amount of coins for a valid command: %p",
    (
  ...
  it.each(exactChangeCases)(
    "should return no change if exact change was provided: %p",
    (testCaseDescription: string, command: string, expectedProduct: string) => {
      expect(getVendingResult(command)).toEqual({
        change: null,
        product: expectedProduct,
      });
  ...
  it.each(insufficientMoneyCases)(
    ...
    (testCaseDescription: string, command: string, expectedChange: string) => {
      expect(getVendingResult(command)).toEqual({
        change: expectedChange,
        product: null,
      });
  ...
  it.each(invalidCommands)(
    "should throw an error containing '%p' with the command '%p'",
    (expectedError: string, command: string) => {
      expect(() => {
